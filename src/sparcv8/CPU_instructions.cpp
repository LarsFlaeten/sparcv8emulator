...
void CPU::LD (pDecode_t d)
{
    ...
    if (d->ev & LOBITS2) {
        ...
        if((load32(d->ev, d->rd, 0, false) < 0) && !mmu.get_no_fault())
...
void CPU::LDUB (pDecode_t d)
{
    ...
    if((load8(d->ev, d->rd, 0, false) < 0) && !mmu.get_no_fault())
        trap(d,  SPARC_DATA_ACCESS_EXCEPTION); 
    else {
        d->pc = d->npc;
        d->npc += 4;
...
void CPU::LDUH (pDecode_t d)
{
    ...
    if (d->ev & LOBITS1) {
        ...
        if((load16(d->ev, d->rd, 0, false) < 0) && !mmu.get_no_fault())
            trap(d,  SPARC_DATA_ACCESS_EXCEPTION); 
        else {
            d->pc = d->npc;
            d->npc += 4;
...
void CPU::LDD (pDecode_t d)
{
    ...
    if (d->rd & LOBITS1) {
        ...
        if((load64(d->ev, d->rd & ~LOBITS1, 0, false) < 0) && !mmu.get_no_fault())
            trap(d,  SPARC_DATA_ACCESS_EXCEPTION); 
        else {
            d->pc = d->npc;
            d->npc += 4;
...
void CPU::LDSB (pDecode_t d)
{
    ...
    if((load8(d->ev, d->rd, 1, false) < 0) && !mmu.get_no_fault())
        trap(d,  SPARC_DATA_ACCESS_EXCEPTION); 
    else {
        d->pc = d->npc;
        d->npc += 4;
...
void CPU::LDSH (pDecode_t d)
{
    ...
    if (d->ev & LOBITS1) {
        ...
        if((load16(d->ev, d->rd, 1, false) < 0) && !mmu.get_no_fault())
            trap(d,  SPARC_DATA_ACCESS_EXCEPTION); 
        else {
            d->pc = d->npc;
            d->npc += 4;
...
void CPU::ST (pDecode_t d)
{
    ...
    if (d->ev & LOBITS2) {
        ...
        if((store32(d->ev, d->rd) < 0) && !mmu.get_no_fault()) {
            trap(d,  SPARC_DATA_ACCESS_EXCEPTION); 
        } else {
            d->pc = d->npc;
            d->npc += 4;
...
void CPU::STB (pDecode_t d)
{
    ...
    if((store8(d->ev, d->rd) < 0) && !mmu.get_no_fault())
        trap(d,  SPARC_DATA_ACCESS_EXCEPTION); 
    else {
        d->pc = d->npc;
        d->npc += 4;
...
void CPU::STH (pDecode_t d)
{
    ...
    if (d->ev & LOBITS1) {
        ...
        if((store16(d->ev, d->rd) < 0) && !mmu.get_no_fault())
            trap(d,  SPARC_DATA_ACCESS_EXCEPTION); 
        else {
            d->pc = d->npc;
            d->npc += 4;
...
void CPU::STD (pDecode_t d)
{
    ...
    if (d->rd & LOBITS1) {
        ...
        if(( store64(d->ev, d->rd) < 0) && !mmu.get_no_fault())
            trap(d,  SPARC_DATA_ACCESS_EXCEPTION); 
        else {
            d->rd &= ~LOBITS1;
            d->pc = d->npc;
...
void CPU::LDSTUB (pDecode_t d)
{
    ...
    if(mmu.get_enabled()) {
        auto translate_res = mmu.translate_va(d->ev, super, intent_load, !mmu.get_no_fault());
        if(!translate_res.ok) {
            trap(d,  SPARC_DATA_ACCESS_EXCEPTION); 
            return;    
        }
    ...
    // Do the ldstub
    auto ares = mmu.get_mctrl().atomic_ldstub8(paddr);
