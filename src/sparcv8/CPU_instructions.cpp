...
void CPU::LD (pDecode_t d)
{
    ...
    if (d->ev & LOBITS2) {
        ...
        if((mem_read(d->ev, 4, d->rd, 0) < 0) && !mmu.GetNoFault())
...
void CPU::LDUB (pDecode_t d)
{
    ...
    if((mem_read(d->ev, 1, d->rd, 0) < 0) && !mmu.GetNoFault())
        trap(d,  SPARC_DATA_ACCESS_EXCEPTION); 
    else {
        d->pc = d->npc;
        d->npc += 4;
...
void CPU::LDUH (pDecode_t d)
{
    ...
    if (d->ev & LOBITS1) {
        ...
        if((mem_read(d->ev, 2, d->rd, 0) < 0) && !mmu.GetNoFault())
            trap(d,  SPARC_DATA_ACCESS_EXCEPTION); 
        else {
            d->pc = d->npc;
            d->npc += 4;
...
void CPU::LDD (pDecode_t d)
{
    ...
    if (d->rd & LOBITS1) {
        ...
        if((mem_read(d->ev, 8, d->rd & ~LOBITS1, 0) < 0) && !mmu.GetNoFault())
            trap(d,  SPARC_DATA_ACCESS_EXCEPTION); 
        else {
            d->pc = d->npc;
            d->npc += 4;
...
void CPU::LDSB (pDecode_t d)
{
    ...
    if((mem_read(d->ev, 1, d->rd, 1) < 0) && !mmu.GetNoFault())
        trap(d,  SPARC_DATA_ACCESS_EXCEPTION); 
    else {
        d->pc = d->npc;
        d->npc += 4;
...
void CPU::LDSH (pDecode_t d)
{
    ...
    if (d->ev & LOBITS1) {
        ...
        if((mem_read(d->ev, 2, d->rd, 1) < 0) && !mmu.GetNoFault())
            trap(d,  SPARC_DATA_ACCESS_EXCEPTION); 
        else {
            d->pc = d->npc;
            d->npc += 4;
